...
| `ORCHESTRATE_IMAGE_MAX_EDGE` | `1568` | Image long-edge downscale (px). |
...
deterministic; only the model's (non-deterministic) visual judgement varies
between runs, so `output.csv` is a valid snapshot rather than a byte-reproducible
artifact. The on-disk cache makes repeated runs reproducible and free.

## Reproduction & provenance

`output.csv` was generated by `python code/main.py` (Batch API) on the few-shot
configuration of this system and is a complete, valid set of 44 predictions.
The latest prompt refinements — prompt-injection spotlighting, XML-structured
prompt, a confidence field, and data-driven guidance (multi-image leniency per
`REQ_GENERAL_MULTI_IMAGE`, issue-type visual definitions, and severity↔support
calibration derived from the labelled set) — improve the code further; a final
re-evaluation and `output.csv` refresh were deferred because the development API
key reached its usage limit. To refresh predictions with the current system,
re-run `python code/main.py`. Re-score the labelled set at zero API cost with
`python code/evaluation/main.py --score-only`.
