...
`output.csv` is a complete, valid set of 44 predictions generated by
`python code/main.py` (Batch API) on the **final** system — few-shot exemplars,
spotlighting prompt-injection defense, and the data-driven prompt refinements
(multi-image leniency per `REQ_GENERAL_MULTI_IMAGE`, issue-type visual
definitions, and severity↔support calibration). Because the model's visual
judgement is non-deterministic, `output.csv` is a valid snapshot rather than a
byte-reproducible artifact; re-running `python code/main.py` refreshes it.
Re-score the labelled set at zero API cost with
